          ...
          {tab === 'providers' && config && <section role="tabpanel" id="panel-providers" aria-labelledby="tab-providers">
            ...
            <fieldset disabled={disabled} className="provider-fields">
              ...
              {['ollama', 'openai-compatible'].includes(config.llmProvider) ? <>
                {config.llmProvider === 'ollama' ? <ModelPicker provider="ollama" endpoint={config.llmBaseUrl ?? defaultEndpoint} value={config.llmModel} effort="none" disabled={disabled} onChange={(llmModel, reasoningEffort) => setConfig({ ...config, llmModel, reasoningEffort })} /> : <label className="field">Model name<input name="llmModel" value={config.llmModel} onChange={event => setConfig({ ...config, llmModel: event.target.value })} placeholder="Model available at your endpoint" /></label>}
              ...
              </> : <>{['copilot', 'codex'].includes(config.llmProvider) ? <ModelPicker key={config.llmProvider} provider={config.llmProvider as 'copilot' | 'codex'} endpoint="" value={config.llmModel} effort={config.reasoningEffort} disabled={disabled} codexApproved={config.codexRestrictedApproved} onChange={(llmModel, reasoningEffort) => setConfig({ ...config, llmModel, reasoningEffort })} /> : <div className="model-summary"><span>Remote model</span><strong>{config.llmModel}</strong>{config.reasoningEffort && config.reasoningEffort !== 'none' && <span>Reasoning effort: {config.reasoningEffort}</span>}</div>}
